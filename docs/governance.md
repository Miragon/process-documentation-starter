...
|---|---|---|
| **Process owner** | Accountable for one process; approves releases | Approves PRs touching `processes/<id>/` — their merge approval *is* the release. Named via `owner.team` (+ `owner.role`) in `process.yaml`, and in the `approval` block once the model is `as-is`. |
...
Most people who *live* a process will never open a pull request — and they don't have to.
